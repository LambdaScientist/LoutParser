...
import qualified HoriGalley as HG
...
instance Show Object where
  show obj =
    ...
    "Delayed Targets: " ++
    (show $ delayedTargets obj) ++ "Object Height: " ++ (show $ height obj)
...
prefix c o = o'
  where
    o' =
      ...
      , height = cHeight c + height o -- length c + height o
      , width = length c + height o
      }
...
fooEx = npageList' 20 # txt purcell


bodycs = splitList $ bodycs' --  encloseBoxQuick "*" <$>
bodycs' = (lout -| lout) : columns left 14 1 " \
        \ In the world of music\
        \ England is supposed\
        \ to be a mere province.\
        \ If she produces an\
